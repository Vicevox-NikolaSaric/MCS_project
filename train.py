...
class NoisyImages(Dataset):
    def __init__(self, path_to_trainset, transform=None):
        self.dataset = read_csv(path_to_trainset, sep=' ')
...
def main():
    ...
    torch.save(model.module.state_dict(), 'saved_models/my_' + str(args.epochs) + '_epoch_' + str(
        len(trainset)) + '_slika_' + args.name + '_checkpoint.pth.tar')


def train(train_loader, model, criterion1, criterion2, optimizer, epoch, args, device, len, file, picture_size):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss = criterion1(output, target / picture_size) + 0.1 * criterion2(output, target / picture_size)
